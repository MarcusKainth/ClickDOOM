//! The `emulation` namespace: the RV32IM CPU that lives in SQL, executing
//! the DOOM ROM.
//!
...
/// Default history file `emulation bench canonical` appends to.
const CANONICAL_RESULTS_DEFAULT: &str = "rom/bench/canonical_throughput/results.jsonl";
/// Default history file `emulation bench compare-versions` appends to.
...
/// Dispatches one `emulation` subcommand.
pub(super) async fn run(cmd: &EmulationCmd) -> Result<Exit, Failure> {
    match &cmd.command {
