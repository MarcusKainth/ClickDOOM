...
(
    SELECT
        ...
        arrayFirstIndex(x -> x >= ofs, e.ends) AS found,
        toUInt32(if(found = 0, ofs, e.ends[found])) AS stop
    FROM {{DB}}.wad_lumps AS l
    CROSS JOIN (SELECT columns, rows FROM {{DB}}.lv_blockmap_header LIMIT 1) AS h
    CROSS JOIN
    (
        -- Where every terminator sits, in word positions. A line number
        -- is never negative, so a -1 anywhere in the lump ends a list and
        -- the first one at or after a cell's offset ends that cell's.
        -- Searching this array costs nothing per cell; searching the lump
        -- itself would copy the lump once per word looked at.
        SELECT groupArray(at) AS ends
        FROM
        (
            SELECT i AS at
            FROM {{DB}}.wad_lumps AS l
            ARRAY JOIN range(toUInt32(intDiv(length(l.bytes), 2))) AS i
            WHERE l.map_marker = '{{MAP}}' AND l.name = 'BLOCKMAP'
              AND reinterpretAsInt16(substring(l.bytes, i * 2 + 1, 2)) = -1
            ORDER BY at
        )
    ) AS e
...
-- Every cell's list has to end at a -1 inside the lump. The check reads
-- the one word past the list the decode kept, so it costs a lookup per
-- cell. A cell whose offset has no terminator after it keeps an empty
-- list, and the word at the offset is the leading zero rather than a -1,
-- so that lands here too.
...
(
    SELECT
        ...
        toUInt32(reinterpretAsUInt16(substring(l.bytes, (4 + b.cell) * 2 + 1, 2)))
            + length(b.lines) AS at
    FROM {{DB}}.lv_blockmap AS b
    CROSS JOIN {{DB}}.wad_lumps AS l
    WHERE l.map_marker = '{{MAP}}' AND l.name = 'BLOCKMAP'
) AS e
WHERE e.at >= e.total OR reinterpretAsInt16(substring(e.bytes, e.at * 2 + 1, 2)) != -1;
...
--
-- A step pushes the branch that reaches the node pushed before it, which
-- is the branch that node's new parent takes to get there. That is what
-- makes `sides[i]` the branch taken at `nodes[i]` rather than the branch
-- that arrived at it.
        ...
        arrayFold(
            (acc, step) -> if(acc.3 = 4294967295,
                              ...
                              (arrayPushBack(acc.1, acc.3),
                               arrayPushBack(acc.2, up.side[acc.1[length(acc.1)] + 1]),
                               up.parent[acc.3 + 1])),
            range(64),
