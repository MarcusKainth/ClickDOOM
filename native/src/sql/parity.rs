//! The differential between the simulation and the reference emulator.
//!
//! Both sides write the same field list, so the comparison is one query per
//! question: which field moves first, and which fields ever move. Each
//! generated query joins `native_state` to the last `probe_state` row of
//! each `gametic` and reports `(tic, kind, slot, field, ours, theirs)`.
//!
//! Five columns are dropped. `refemu/probe/README.md` says why the probe
//! cannot read an identity, and comparing by slot is what stands in for it.
