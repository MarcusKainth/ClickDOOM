...
fn removed(state: &State, player: &str) -> Vec<(String, String)> {
    ...
    for column in super::state_columns() {
        ...
        // `P_AddThinker` puts a new thing on the end of the list, so a
        // fireball the tic threw, the drop its claw's kill left, the puffs
        // and blood its gun attackers' shots left and an in-flight
        // missile's own drop go behind what survived the compaction, in
        // that order.
        let segment =
        ...
        let born = format!(
            ...
            segment(
                ...
                "mt_missile_drops",
                "d",
                "length(mt_thrown) + length(mt_drops) + length(mt_debris)"
            ),
        );
        bind(
            &format!("now_{column}"),
    ...
    // Every thing the tic threw, dropped or left as debris took one of
    // each counter.
    for column in ["next_seq", "next_linkseq"] {
        bind(
            &format!("now_{column}"),
            format!(
                "toUInt32({} + length(mt_thrown) + length(mt_drops) + length(mt_debris) \
                 + length(mt_missile_drops))",
                s(column)
            ),
        );
    }
...
fn strikes(state: &State, map: &World<'_>) -> Vec<(String, String)> {
    ...
    // order. `spawn_debris`'s own `born` tuples join the mobj list behind
    // `mt_thrown`'s and `mt_drops`'s. `AT_SPAWN_GUNS` below refuses the
    // tic where the stage spawns any of the other two beside them, so
    // their relative order there never has to be decided.
    bind(
        "at_gun_debris_asks",
        format!(
    ...
    bind(
        ...
        mask(
            ...
            &[
                ...
                (
                    unresolved::AT_SPAWN_GUNS,
                    &format!(
                        "(at_struck.{} = 1 OR mt_hurt.{} != -1) AND length(at_gun) > 0",
                        attacks::attacked::THROWS,
                        inter::hurt::DROP,
                    ),
                ),
