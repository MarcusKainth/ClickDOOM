...
/// The level loaded, with nothing run yet.
...
/// The demo drives this plat's tics, and the demo lump is read by
/// absolute tic, so it runs in a session of its own.
#[tokio::test]
async fn a_seeded_plat_runs_the_way_the_engine_runs_it() {
    let fixture = load_level("sim_plat").await;
    let db = fixture.database.clone();
    let mut session = Session::open(&fixture, false).await;
    session.feed(&[Input::demo(1)]).await;
    ...
    let rows: Vec<Ran> = fixture
        .rows(&format!(
            "SELECT tic, sec_floorheight[{}] AS floor, s_status[17] AS status, \
             ...
             FROM {db}.native_state WHERE tic BETWEEN {} AND {} ORDER BY tic",
            SECTOR + 1,
            SEED_TIC + 1,
            SEED_TIC + TICS
        ))
...
/// Where each crossing arm's seeded row lands.
const WR_AT: u32 = 900;
const W1_AT: u32 = 1000;
const RAISE_AT: u32 = 1100;
const MONSTER_AT: u32 = 1300;

/// A column of the slot `slot` names replaced, leaving every other slot
/// alone.
fn put(slot: &str, column: &'static str, value: String) -> (&'static str, String) {
    (
        column,
        format!(
            "arrayMap((v, k) -> if(k = {slot}, {value}, v), \
             p.{column}, arrayEnumerate(p.{column}))"
        ),
    )
}

/// An arm seeded from tic 1 at `at` that runs `tics` tics with no input.
fn crossing(name: &'static str, at: u32, tics: u32, overrides: Vec<(&'static str, String)>) -> Arm {
    Arm {
        name,
        from: 1,
        overrides,
...
/// The player put across line 396.
fn line_396(slot: &str) -> Vec<(&'static str, String)> {
    vec![
        put(slot, "m_x", format!("toInt32({CROSS_OLD_X})")),
        put(slot, "m_y", format!("toInt32({CROSS_OLD_Y})")),
        put(slot, "m_momx", format!("toInt32({CROSS_MOMX})")),
        put(slot, "m_momy", format!("toInt32({CROSS_MOMY})")),
        put(slot, "m_z", format!("toInt32({CROSS_FLOORZ})")),
        put(slot, "m_floorz", format!("toInt32({CROSS_FLOORZ})")),
        put(slot, "m_ceilingz", format!("toInt32({CROSS_CEILINGZ})")),
    ]
}
...
/// The thing in `slot` put across line 511.
fn line_511(slot: &str) -> Vec<(&'static str, String)> {
    vec![
        put(slot, "m_x", format!("toInt32({RAISE_OLD_X})")),
        put(slot, "m_y", format!("toInt32({RAISE_OLD_Y})")),
        put(slot, "m_momx", format!("toInt32({RAISE_MOMX})")),
        put(slot, "m_momy", format!("toInt32({RAISE_MOMY})")),
        put(slot, "m_z", format!("toInt32({RAISE_FLOORZ})")),
        put(slot, "m_floorz", format!("toInt32({RAISE_FLOORZ})")),
        put(slot, "m_ceilingz", format!("toInt32({RAISE_CEILINGZ})")),
    ]
}
...
/// Long enough for the thrust to carry the monster over the line, and for
/// a plat to have shown itself if one were spawned.
const MONSTER_TICS: u32 = 20;

/// The first live zombieman.
...
/// The rows a crossing of line 396 reads: the plat tagged 1 and sector 23.
fn crossed_columns() -> String {
    format!(
        "tic, \
...
/// The same, with line 397's own special.
...
/// The rows a crossing of line 511 reads: the plat tagged 7 and sector
/// 120.
fn raised_columns() -> String {
    format!(
        "tic, \
...
/// Every crossing arm, seeded from tic 1 and driven through one session.
#[tokio::test]
async fn a_crossing_spawns_the_plat_ev_do_plat_spawns_for_it() {
    let fixture = load_level("sim_plat_cross").await;
    let mut one_shot = line_396("p.p_mo");
    one_shot.push((
        "line_special",
    ...
    let arms = Arms::new(
        vec![Input::demo(1)],
        vec![
            crossing("wr", WR_AT, CROSS_TICS, line_396("p.p_mo")),
            crossing("w1", W1_AT, CROSS_TICS, one_shot),
            crossing("raise", RAISE_AT, RAISE_TICS, line_511("p.p_mo")),
            crossing("monster", MONSTER_AT, MONSTER_TICS, line_511(MONSTER)),
        ],
    );
    let mut session = Session::open(&fixture, false).await;
    ...
    let arm = arms.arm("wr");
    let rows: Vec<Crossed> = arm
        .rows(&fixture, "native_state", &crossed_columns())
        .await
        .into_iter()
        .filter(|row: &Crossed| row.tic > arm.at)
        .collect();
    a_crossing_of_the_tagged_line_spawns_the_plat(checks.arm("wr"), &rows);

    let arm = arms.arm("w1");
    let rows: Vec<CrossedOneShot> = arm
        .rows(&fixture, "native_state", &one_shot_columns())
        .await
        .into_iter()
        .filter(|row: &CrossedOneShot| row.tic > arm.at)
        .collect();
    a_crossing_of_a_one_shot_line_spawns_the_plat_and_clears_the_line(checks.arm("w1"), &rows);

    let arm = arms.arm("raise");
    let rows: Vec<Raised> = arm
        .rows(&fixture, "native_state", &raised_columns())
        .await
        .into_iter()
        .filter(|row: &Raised| row.tic > arm.at)
        .collect();
    a_crossing_of_the_type_22_line_spawns_the_raise_to_nearest_and_change_plat(
        checks.arm("raise"),
        &rows,
    );

    let arm = arms.arm("monster");
    let rows: Vec<Raised> = arm
        .rows(&fixture, "native_state", &raised_columns())
        .await
        .into_iter()
        .filter(|row: &Raised| row.tic > arm.at)
        .collect();
    a_monsters_crossing_of_the_type_22_line_spawns_nothing(checks.arm("monster"), &rows);
...
fn a_crossing_of_the_tagged_line_spawns_the_plat(mut c: ArmChecks<'_>, rows: &[Crossed]) {
    c.eq(rows.len(), CROSS_TICS as usize, "every tic ran");
    ...
    let Some(spawned) = rows.iter().find(|row| row.slot != 0) else {
        c.check(false, "no tic after the crossing spawns the plat");
        return;
    };
    ...
    c.check(want.reached_bottom, "the run reaches the bottom");
}

/// The same crossing of line 396 as the WR arm, with the line's own
...
fn a_crossing_of_a_one_shot_line_spawns_the_plat_and_clears_the_line(
    mut c: ArmChecks<'_>,
    rows: &[CrossedOneShot],
) {
    c.eq(rows.len(), CROSS_TICS as usize, "every tic ran");
    ...
    for row in rows.iter().filter(|row| row.tic > spawned.tic) {
        ...
        c.eq(
            row.special,
            0,
            format!("tic {}: the line stays cleared", row.tic),
        );
    }
    c.check(want.reached_bottom, "the run reaches the bottom");
}
...
fn a_crossing_of_the_type_22_line_spawns_the_raise_to_nearest_and_change_plat(
    mut c: ArmChecks<'_>,
    rows: &[Raised],
) {
    c.eq(rows.len(), RAISE_TICS as usize, "every tic ran");
    ...
    for row in rows.iter().filter(|row| row.tic > spawned.tic) {
        if want.done {
            ...
            c.eq(row.slot, 0, format!("tic {} keeps the plat gone", row.tic));
            c.eq(
                row.floor,
                RAISED_HIGH,
                format!("tic {} holds the arrived floor", row.tic),
            );
            continue;
        }
        let step = want.tic();
        if want.done {
            c.eq(row.slot, 0, format!("tic {} removes the plat", row.tic));
            c.eq(row.floor, step.floorheight, format!("tic {}", row.tic));
        } else {
            c.eq(
                (row.floor, row.status, row.count),
                (step.floorheight, step.status, step.count),
    ...
    c.check(want.done, "the run reaches the top and comes off the list");
}
...
/// rather than the player: the first live zombieman, given the player's
/// own momentum. A thrust is what carries a monster over a line, so the
/// momentum is what makes this a crossing at all.
fn a_monsters_crossing_of_the_type_22_line_spawns_nothing(mut c: ArmChecks<'_>, rows: &[Raised]) {
    c.eq(rows.len(), MONSTER_TICS as usize, "every tic ran");
    for row in rows {
        c.eq(
            row.slot,
            0,
        ...
        c.eq(
            row.floor,
            RAISED_FLOOR,
            format!("tic {} moved the sector's own floor", row.tic),
        );
        c.check(
            row.special != 0,
            format!(
...
/// emptying. The demo drives the tics, so this runs in a session of its
/// own.
#[tokio::test]
async fn a_seeded_button_puts_its_picture_back_when_it_runs_out() {
    let fixture = load_level("sim_button").await;
    let db = fixture.database.clone();
    let mut session = Session::open(&fixture, false).await;
    session.feed(&[Input::demo(1)]).await;

    let button = |column: &'static str, value: String| {
        (
            column,
            format!(
    ...
    let seeded: Vec<sql::Statement> = support::seed::row(
        &db,
        SEED_TIC,
        1,
        &[
            button("btn_timer", format!("toInt32({BUTTONTIME})")),
            button("btn_line", format!("toInt32({BUTTON_LINE})")),
            button("btn_where", "toInt32(0)".to_owned()),
            button("btn_texture", format!("toInt32({BUTTON_TEXTURE})")),
        ],
    )
    .into_iter()
    .map(sql::Statement::sql)
    ...
    let last = SEED_TIC + BUTTONTIME as u32 + 2;
    let walk: Vec<Input> = (SEED_TIC + 1..=last).map(Input::demo).collect();
    ...
    let rows: Vec<Pressed> = fixture
        .rows(&format!(
            "SELECT tic, btn_timer[1] AS timer, btn_line[1] AS line, \
             ...
             WHERE tic BETWEEN {} AND {last} ORDER BY tic",
            SEED_TIC + 1
        ))
        .await;
    fixture.finish().await;
