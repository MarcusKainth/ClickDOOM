...
/// Lifts the target clear of `P_AimLineAttack`'s own cone. The cone runs
/// to `100 * FRACUNIT / 160` either side of `shootz`, which at the
/// target's distance of about 2,061,552 reaches 1,288,470 above it, and
/// `shootz` itself stands 2,359,296 over the player's own `m_z`. A target
/// whose bottom sits this far up is outside the cone, so the aim reaches
/// nothing and answers a slope of 0, and the attack along that slope
/// passes underneath.
const ABOVE_THE_CONE: i64 = 8_000_000;
...
struct Arm {
    ...
    in_range: bool,
    berserk: bool,
    /// Lifts the target out of the aim's cone while leaving it in range.
    above_the_cone: bool,
}
...
async fn a_punch_lands_only_where_the_engine_lands_it() {
    ...
    for arm in &arms {
        ...
        let lift = if arm.above_the_cone {
            ABOVE_THE_CONE
        } else {
            0
        };
            ...
            (
                ...
                format!(
                    "arrayMap((v, k) -> toInt32(if(k = ((p.p_mo % length(p.m_state)) + 1), \
                     p.m_z[p.p_mo] + {lift}, v)), p.m_z, arrayEnumerate(p.m_z))"
                ),
            ),
            (
                "m_flags",
    ...
    // `A_Punch` turns towards `linetarget`, which `P_AimLineAttack` sets
    // and `P_LineAttack` never writes. The target here is in range and
    // shootable, and the aim still answers nothing, so neither the turn
    // nor the damage happens.
    let above = at("above_the_cone");
    assert_eq!(above.unresolved, 0);
    assert_eq!(
        above.m_angle[p_mo - 1],
        0,
        "a punch whose aim reached nothing leaves the angle alone, target or no target"
    );
    assert_eq!(
        above.m_health[target_slot - 1],
        TARGET_HEALTH as i32,
        "the attack passes under a target the aim could not take"
    );
}
