//! `A_ReFire` and `A_FireCGun`, on a real ClickHouse server.
//!
//! Each is seeded straight from the level's own first row rather than run
//! forward from a demo, since every field the two read (the readied
//! weapon, its ammunition, the state cycle) is one this overrides.
//!
//! Needs a reachable ClickHouse (`CLICKHOUSE_HOST` / `CLICKHOUSE_HTTP_PORT`
//! / `CLICKHOUSE_PASSWORD`, defaulting to `localhost:8123` with no
//! password). Behind the `clickhouse-tests` feature, so a run without a
//! server visibly excludes them.
#![cfg(feature = "clickhouse-tests")]

use clickdoom_native::sql::sim;
use clickdoom_native::sql::sim::tick::Input;
use clickdoom_native::{load, sql, wad::Wad};
use clickdoom_spec::native_state::key;
use clickhouse::Row;
use serde::Deserialize;

mod support;

use support::arms::{Arm, ArmChecks, Arms, Checks};
use support::db::Fixture;
use support::resident::Session;

/// `info.c`: the pistol's own state cycle.
const S_PISTOL1: i32 = 13;
const S_PISTOL2: i32 = 14;
const S_PISTOL3: i32 = 15;
const S_PISTOL4: i32 = 16;

/// `info.c`: the chaingun's own state cycle.
const S_CHAIN: i32 = 49;
const S_CHAIN1: i32 = 52;
const S_CHAIN2: i32 = 53;
const S_CHAINFLASH1: i32 = 55;
const S_CHAINFLASH2: i32 = 56;

/// `doomdef.h`
const WP_PISTOL: i32 = 1;
const WP_CHAINGUN: i32 = 3;
const WP_NOCHANGE: i32 = 10;
/// `d_items.c`: both the pistol and the chaingun eat `am_clip`, one-based
/// for `p_ammo`.
const AM_CLIP: usize = 1;

/// One trigger arm: its name, where the copy of tic 0 lands, the psprite
/// state it seeds the weapon sprite to, the ammunition it leaves in the
/// clip, and whether the fire key is down for the tic that runs from it.
struct Trigger {
    name: &'static str,
    at: u32,
    weapon: i32,
    state: i32,
    ammo: i32,
    fires: bool,
}

/// `A_ReFire`'s arms are seeded at `S_PISTOL3`, one tic from its own
/// `nextstate`, so the tic this runs is the one `A_ReFire` (`S_PISTOL4`)
/// itself decides. `A_FireCGun`'s are seeded at `S_CHAIN1` (the burst's
/// first frame) so the tic advances into `S_CHAIN2`, the burst's second,
/// where `A_FireCGun` runs again. The ready state fires the burst's own
/// first shot, `S_CHAIN1` itself, in the same tic `A_WeaponReady`
/// redirects into it.
const TRIGGERS: [Trigger; 6] = [
    Trigger {
        name: "refires",
        at: 200,
        weapon: WP_PISTOL,
        state: S_PISTOL3,
        ammo: 50,
        fires: true,
    },
    Trigger {
        name: "stops",
        at: 300,
        weapon: WP_PISTOL,
        state: S_PISTOL3,
        ammo: 50,
        fires: false,
    },
    Trigger {
        name: "empty",
        at: 400,
        weapon: WP_PISTOL,
        state: S_PISTOL3,
        ammo: 0,
        fires: true,
    },
    Trigger {
        name: "second_shot_fires",
        at: 500,
        weapon: WP_CHAINGUN,
        state: S_CHAIN1,
        ammo: 5,
        fires: true,
    },
    Trigger {
        name: "clip_ran_out",
        at: 600,
        weapon: WP_CHAINGUN,
        state: S_CHAIN1,
        ammo: 0,
        fires: true,
    },
    Trigger {
        name: "first_shot_fires",
        at: 700,
        weapon: WP_CHAINGUN,
        state: S_CHAIN,
        ammo: 5,
        fires: true,
    },
];

fn trigger(arm: &Trigger) -> Arm {
    let overrides = vec![
        ("p_readyweapon", format!("toInt32({})", arm.weapon)),
        ("p_pendingweapon", format!("toInt32({WP_NOCHANGE})")),
        (
            "psp_state",
            format!("CAST([{}, -1], 'Array(Int32)')", arm.state),
        ),
        ("psp_tics", "CAST([1, -1], 'Array(Int32)')".to_owned()),
        (
            "p_ammo",
            format!(
                "arrayMap((v, k) -> toInt32(if(k = {AM_CLIP}, {}, v)), \
                 p.p_ammo, arrayEnumerate(p.p_ammo))",
                arm.ammo
            ),
        ),
        ("p_refire", "toInt32(0)".to_owned()),
        ("p_health", "toInt32(100)".to_owned()),
        ("p_extralight", "toInt32(0)".to_owned()),
    ];
    let keys = if arm.fires { key::FIRE } else { 0 };
    Arm {
        name: arm.name,
        from: 0,
        overrides,
        at: arm.at,
        inputs: vec![Input::keys(arm.at + 1, keys, (0, 0))],
    }
}

/// The pistol's accuracy arms: their names, where the copy of tic 0 lands,
/// and the `p_refire` each is seeded with. The third is a second held shot
/// in a row rather than the first: `A_FirePistol` never touches
/// `player->refire` itself, so nothing here is expected to move it from 2.
const SHOTS: [(&str, u32, i32); 3] = [("accurate", 800, 0), ("spread", 900, 1), ("third", 1000, 2)];

/// Seeded at `S_PISTOL1` so the tic advances into `S_PISTOL2`, where
/// `A_FirePistol` itself runs; `p_refire` is the one field the arms differ
/// by.
fn shot((name, at, refire): (&'static str, u32, i32)) -> Arm {
    let overrides = vec![
        ("p_readyweapon", format!("toInt32({WP_PISTOL})")),
        ("p_pendingweapon", format!("toInt32({WP_NOCHANGE})")),
        (
            "psp_state",
            format!("CAST([{S_PISTOL1}, -1], 'Array(Int32)')"),
        ),
        ("psp_tics", "CAST([1, -1], 'Array(Int32)')".to_owned()),
        (
            "p_ammo",
            format!(
                "arrayMap((v, k) -> toInt32(if(k = {AM_CLIP}, 50, v)), \
                 p.p_ammo, arrayEnumerate(p.p_ammo))"
            ),
        ),
        // Every other mobj is stopped outright, tics included, so
        // nothing but the shot itself draws a number this tic: a
        // thinker still standing would run its own actions and blur
        // the count this checks. Moved off too, since a trace tests
        // every mobj it is given whatever its own state carries.
        (
            "m_state",
            "arrayMap((v, k) -> toInt32(if(k = p.p_mo, v, 0)), \
             p.m_state, arrayEnumerate(p.m_state))"
                .to_owned(),
        ),
        (
            "m_tics",
            "arrayMap((v, k) -> toInt32(if(k = p.p_mo, v, -1)), \
             p.m_tics, arrayEnumerate(p.m_tics))"
                .to_owned(),
        ),
        (
            "m_x",
            "arrayMap((v, k) -> toInt32(if(k = p.p_mo, v, -2000000000)), \
             p.m_x, arrayEnumerate(p.m_x))"
                .to_owned(),
        ),
        ("p_refire", format!("toInt32({refire})")),
        ("p_health", "toInt32(100)".to_owned()),
    ];
    Arm {
        name,
        from: 0,
        overrides,
        at,
        inputs: vec![Input::keys(at + 1, key::FIRE, (0, 0))],
    }
}

#[derive(Row, Deserialize)]
struct Ran {
    tic: u32,
    psp_state: Vec<i32>,
    ammo: i32,
    refire: i32,
    extralight: i32,
    unresolved: u64,
}

fn ran_columns() -> String {
    format!(
        "tic, psp_state, p_ammo[{AM_CLIP}] AS ammo, p_refire AS refire, \
         p_extralight AS extralight, unresolved"
    )
}

#[derive(Row, Deserialize)]
struct Fired {
    tic: u32,
    psp_state: Vec<i32>,
    prndindex: u8,
    unresolved: u64,
    next_seq: u32,
    mobj_count: u64,
    refire: i32,
}

const FIRED: &str = "tic, psp_state, prndindex, unresolved, next_seq, \
                     length(m_state) AS mobj_count, p_refire AS refire";

/// The row a trigger arm's one tic left, or `None` with a failed check
/// when there is none.
async fn ran(fixture: &Fixture, arm: &Arm, c: &mut ArmChecks<'_>) -> Option<Ran> {
    let row = arm
        .rows::<Ran>(fixture, "native_state", &ran_columns())
        .await
        .into_iter()
        .find(|row| row.tic == arm.at + 1);
    c.check(row.is_some(), "the arm ran");
    row
}

/// The row a shot arm's one tic left.
async fn fired(fixture: &Fixture, arm: &Arm) -> Option<Fired> {
    arm.rows::<Fired>(fixture, "native_state", FIRED)
        .await
        .into_iter()
        .find(|row| row.tic == arm.at + 1)
}

#[tokio::test]
async fn a_pistol_and_a_chaingun_fire_the_way_the_engine_fires_them() {
    let bytes = support::doom1();
    let wad = Wad::parse(&bytes).unwrap();
    let fixture = Fixture::create("sim_refire").await;
    let db = fixture.database.clone();

    let mut plan = load::plan(&db, &wad);
    plan.extend(sql::level_statements(&db, support::MAP, support::DEMO));
    plan.extend(sim::load_statements(&db));
    if let Err(error) = fixture.execute(&plan).await {
        fixture.finish().await;
        panic!("{error}");
    }

    let mut seeded: Vec<Arm> = TRIGGERS.iter().map(trigger).collect();
    seeded.extend(SHOTS.into_iter().map(shot));
    let arms = Arms::new(Vec::new(), seeded);
    let mut session = Session::open(&fixture, false).await;
    arms.drive(&fixture, &mut session).await;
    session.close().await;

    let mut checks = arms.checks();
    a_refire_counts_while_the_trigger_stays_down(&fixture, &arms, &mut checks).await;
    a_held_pistols_second_shot_spreads(&fixture, &arms, &mut checks).await;
    a_fire_cgun_stops_mid_burst_when_the_clip_runs_out(&fixture, &arms, &mut checks).await;
    fixture.finish().await;
    checks.finish();
}

/// `A_ReFire`'s own two branches: it fires again and counts `refire` up
/// while the trigger stays down, and drops back to 0 the tic it does not,
/// whether that is because the trigger came up or because the clip ran
/// dry in between.
async fn a_refire_counts_while_the_trigger_stays_down(
    fixture: &Fixture,
    arms: &Arms,
    checks: &mut Checks,
) {
    let mut c = checks.arm("refires");
    if let Some(refires) = ran(fixture, arms.arm("refires"), &mut c).await {
        c.eq(
            (refires.psp_state[0], refires.refire, refires.unresolved),
            (S_PISTOL1, 1, 0),
            "a held trigger fires again and counts the refire up",
        );
    }

    // `A_ReFire` does not call `P_SetPsprite` on its own account, so a tic
    // that does not fire again leaves the sprite exactly where entering
    // `S_PISTOL4` this tic already put it, waiting out its own tics
    // before `S_PISTOL`'s ready state is a cycle away.
    let mut c = checks.arm("stops");
    if let Some(stops) = ran(fixture, arms.arm("stops"), &mut c).await {
        c.eq(
            (stops.psp_state[0], stops.refire, stops.unresolved),
            (S_PISTOL4, 0, 0),
            "a trigger let go of stays on the frame it entered and the count resets",
        );
    }

    let mut c = checks.arm("empty");
    if let Some(empty) = ran(fixture, arms.arm("empty"), &mut c).await {
        c.eq(
            empty.unresolved,
            sim::unresolved::PSP_STUCK,
            "an empty clip reaches P_CheckAmmo's own weapon change, which this does not write",
        );
    }
}

/// `A_FirePistol`'s own accuracy: `!player->refire`, read from the count
/// `A_ReFire`'s own entry left rather than from the tic's starting row. A
/// third held shot spreads exactly as the second does and leaves the
/// count where it found it, since only `A_ReFire` ever writes it.
async fn a_held_pistols_second_shot_spreads(fixture: &Fixture, arms: &Arms, checks: &mut Checks) {
    let mut rows: Vec<Option<Fired>> = Vec::new();
    for (name, _, _) in SHOTS {
        rows.push(fired(fixture, arms.arm(name)).await);
    }
    let accurate = rows[0].as_ref();
    for ((name, _, _), row) in SHOTS.into_iter().zip(&rows) {
        let mut c = checks.arm(name);
        let Some(row) = row else {
            c.check(false, "the arm ran");
            continue;
        };
        c.eq(row.psp_state[0], S_PISTOL2, "the shot fires");
        c.eq(row.unresolved, 0, "unresolved");
        let Some(accurate) = accurate else {
            continue;
        };
        // Every arm is seeded from the same row and fires at the same wall
        // or thing, so whatever the shot itself reaches is identical
        // between them; only the two extra numbers a spread angle draws
        // tell an accurate shot from a spread one.
        match name {
            "spread" => {
                c.eq(
                    (accurate.next_seq, accurate.mobj_count),
                    (row.next_seq, row.mobj_count),
                    "every arm fires the same shot into the same spot",
                );
                c.eq(
                    row.prndindex.wrapping_sub(accurate.prndindex),
                    2,
                    "a held trigger's second shot spreads and draws the angle's own two \
                     numbers, which an unheld trigger's first shot does not",
                );
            }
            "third" => {
                c.eq(
                    (accurate.next_seq, accurate.mobj_count),
                    (row.next_seq, row.mobj_count),
                    "the third arm too",
                );
                c.eq(
                    row.prndindex.wrapping_sub(accurate.prndindex),
                    2,
                    "a third shot in a row spreads exactly the same as the second does",
                );
                c.eq(
                    row.refire,
                    2,
                    "A_FirePistol never touches player->refire itself, so a third \
                     held shot leaves the count exactly where A_ReFire left it",
                );
            }
            _ => {}
        }
    }
}

/// `A_FireCGun`'s own two firing frames share the routine, and only the
/// first is behind `P_CheckAmmo`: the clip can run out between them, and
/// the second returns without firing, decrementing ammunition, or putting
/// the flash sprite anywhere.
async fn a_fire_cgun_stops_mid_burst_when_the_clip_runs_out(
    fixture: &Fixture,
    arms: &Arms,
    checks: &mut Checks,
) {
    let mut c = checks.arm("first_shot_fires");
    if let Some(first) = ran(fixture, arms.arm("first_shot_fires"), &mut c).await {
        c.eq(
            (first.psp_state[0], first.ammo, first.unresolved),
            (S_CHAIN1, 4, 0),
            "the burst's first shot fires from the ready state and spends a round",
        );
        c.eq(
            first.psp_state[1],
            S_CHAINFLASH1,
            "and flashes the burst's own first frame",
        );
        c.eq(first.extralight, 1, "S_CHAINFLASH1 carries A_Light1");
    }

    let mut c = checks.arm("second_shot_fires");
    if let Some(second) = ran(fixture, arms.arm("second_shot_fires"), &mut c).await {
        c.eq(
            (second.psp_state[0], second.ammo, second.unresolved),
            (S_CHAIN2, 4, 0),
            "the burst's second shot fires on its own frame and spends a round",
        );
        c.eq(
            second.psp_state[1],
            S_CHAINFLASH2,
            "and flashes the burst's own second frame, not the first's",
        );
        c.eq(second.extralight, 2, "S_CHAINFLASH2 carries A_Light2");
    }

    let mut c = checks.arm("clip_ran_out");
    if let Some(empty) = ran(fixture, arms.arm("clip_ran_out"), &mut c).await {
        c.eq(
            (empty.psp_state[0], empty.ammo, empty.unresolved),
            (S_CHAIN2, 0, 0),
            "the second shot advances the sprite without firing, spending \
             nothing and leaving the tic resolved",
        );
        c.eq(
            empty.psp_state[1],
            -1,
            "and puts nothing in the flash sprite",
        );
        c.eq(
            empty.extralight,
            0,
            "a dry burst does not brighten the screen",
        );
    }
}
