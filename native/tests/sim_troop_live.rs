...
/// how far the attacker stands from its target.
///
...
fn arms() -> Arms {
    let arms = ARMS
        ...
        .map(|(name, at, away)| {
            ...
            match stands_by {
                // The bystander stands `ALONG` the shot from the attacker,
                // on the target's own floor. A zombieman rather than a
                // third imp, because `PIT_CheckThing` lets a missile
                // through a thing of its own shooter's type without
                // damaging it, and at one point of health, so the
                // fireball's own roll always kills.
...
async fn a_tic_carries_the_imps_attack_through() {
    ...
    let columns = format!(
        "tic, m_state[{ATTACKER}] AS state, m_angle[{ATTACKER}] AS angle, \
    ...
    let mut ran: Vec<Vec<Clawed>> = Vec::new();
    for arm in arms.all() {
    ...
    let (before, after) = pair("claw");
    let mut arm = checks.arm("claw");
    arm.eq(
        before.state,
        FACE,
        "the seeded row is a tic from the routine",
    );
    arm.eq(after.state, ATTACK, "and the cycle reaches it");
    ...
    arm.check(
        ...
        format_args!("the damage is three times one to eight: {taken}"),
    );
    arm.check(
    ...
    // it.
    arm.eq(after.angle, DUE_WEST, "the attacker turns onto its target");
    ...
    // list.
    let (before, after) = pair("fireball");
    let mut arm = checks.arm("fireball");
    arm.eq(after.state, ATTACK, "the cycle reaches the routine");
    arm.eq(
        after.health,
        before.health,
    ...
    let (before, after) = pair("ambush");
    let mut arm = checks.arm("ambush");
    arm.eq(
    ...
    arm.check(before.health - after.health > 0, "and the claw still lands");
    arm.eq(
        (after.target_sprite as i64, after.target_frame as i64),
        support::damage::picture(after.target_state as i64),
    ...
    // already on the list.
    let (before, after) = pair("thrown_hit");
    let mut arm = checks.arm("thrown_hit");
    arm.eq(before.bystander_health, 1, "the seeded row stands it up");
    arm.eq(
